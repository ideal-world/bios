...
impl BIOSMQClient {
    ...
    pub async fn init(str_url: &str) -> BIOSResult<BIOSMQClient> {
        ...
        let con = Connection::connect(str_url, ConnectionProperties::default().with_connection_name("bios".into())).await?;
        info!(
    ...
    pub async fn request(&mut self, address: &str, message: String, header: &HashMap<String, String>) -> BIOSResult<()> {
        trace!("[BIOS.Framework.MQClient] Request, queue:{}, message:{}", address, message);
    ...
    pub async fn publish(&mut self, topic: &str, message: String, header: &HashMap<String, String>) -> BIOSResult<()> {
        trace!("[BIOS.Framework.MQClient] Publish, queue:{}, message:{}", topic, message);
        let channel = self.con.create_channel().await?;
        channel.confirm_select(ConfirmSelectOptions::default()).await?;
        let mut mq_header = FieldTable::default();
        for (k, v) in header {
    ...
    async fn process<F, T>(&mut self, topic_or_address: String, mut consumer: Consumer, mut fun: F) -> BIOSResult<()>
    where
        F: FnMut((HashMap<String, String>, String)) -> T + Send + Sync + 'static,
        T: Future<Output = BIOSResult<()>> + Send + 'static,
    {
            ...
            while let Some(delivery) = consumer.next().await {
                match delivery {
                    Ok((_, info)) => match std::str::from_utf8(&info.data) {
                        Ok(msg) => {
                            trace!("[BIOS.Framework.MQClient] Receive, queue:{}, message:{}", topic_or_address, msg);
                            ...
                            match fun((resp_header, msg.to_string())).await {
                                Ok(_) => match info.ack(BasicAckOptions::default()).await {
                                    Ok(_) => (),
                                    Err(e) => {
                                        error!("[BIOS.Framework.MQClient] Receive, queue:{}, message:{} | {}", topic_or_address, msg, e.to_string());
                                    }
                                },
                                Err(e) => {
                                    error!("[BIOS.Framework.MQClient] Receive, queue:{}, message:{} | {}", topic_or_address, msg, e.to_string());
                        ...
                        Err(e) => {
                            error!("[BIOS.Framework.MQClient] Receive, queue:{} | {}", topic_or_address, e.to_string());
                        }
                    },
                    Err(e) => {
                        error!("[BIOS.Framework.MQClient] Receive, queue:{} | {}", topic_or_address, e.to_string());
