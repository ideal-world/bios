...
impl BIOSUri {
    pub fn format_with_item(&self, host: &str, path_and_query: &str) -> BIOSResult<String> {
    ...
    pub fn format(&self, uri_str: &str) -> BIOSResult<String> {
    ...
    pub fn get_path_and_query(&self, uri_str: &str) -> BIOSResult<String> {
        let uri_result = url::Url::parse(uri_str);
        if uri_result.is_err() {
            return Err(BIOSError::FormatError(uri_result.err().unwrap().to_string()));
        }
