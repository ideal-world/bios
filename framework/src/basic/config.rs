...
where
    T: Deserialize<'a>,
{
    pub(crate) fn init(relative_path: &str) -> BIOSResult<BIOSConfig<T>> {
        ...
        info!(
            "[BIOS.Framework.Config] Initializing, base path:{:?}, relative path:{:?}, profile:{}",
        ...
        info!(
            "[BIOS.Framework.Config] Initialized, base path:{:?}, relative path:{:?}, profile:{}",
            parent_path, relative_path, profile
        );
