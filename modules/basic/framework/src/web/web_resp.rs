...
fn error_handler(err: poem::Error) -> Response {
    let (code, msg) = if err.is::<ParseParamError>() || err.is::<ParseJsonError>() || err.is::<ParseMultipartError>() {
        (StatusCodeKind::BadRequest.to_string(), err.to_string())
    } else if err.is::<ContentTypeError>() {
        (StatusCodeKind::NotFound.to_string(), err.to_string())
    } else if err.is::<AuthorizationError>() {
