...
use super::{IpTimePlugin, SgFilterIpTimeConfig};

use tardis::{chrono::Local, serde_json};
#[test]
fn parse_config() {
    let json_str = include_str!("tests/testconfig.json");
    ...
    let filters = value.into_iter().map(IpTimePlugin::from).collect::<Vec<_>>();
