...
async fn test_app_ident() -> BIOSResult<()> {
    ...
    let resp = call_service(&app, req).await;
    assert_eq!(resp.status(), StatusCode::OK);
    let body = read_body_json::<BIOSRespHelper<Vec<AppIdentDetailResp>>, AnyBody>(resp).await.body.unwrap();
    ...
    let req = test::TestRequest::get()
        ...
        .uri(format!("/console/app/app/ident/{}/sk", id.clone()).as_str())
        .to_request();
    let resp = call_service(&app, req).await;
    assert_eq!(resp.status(), StatusCode::OK);
    let sk = read_body_json::<BIOSRespHelper<String>, AnyBody>(resp).await.body.unwrap();
    assert_ne!(sk, "");
