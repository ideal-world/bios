...
impl IamCertOAuth2Supplier {
    pub fn parse(kind: &str) -> TardisResult<IamCertOAuth2Supplier> {
        IamCertOAuth2Supplier::from_str(kind).map_err(|_| TardisError::format_error(&format!("not supports OAuth2 kind: {kind}",), "404-iam-cert-oauth-kind-not-exist"))
    }
}
...
impl IamSetCateKind {
    pub fn parse(kind: &str) -> TardisResult<IamSetCateKind> {
        IamSetCateKind::from_str(kind).map_err(|_| TardisError::format_error(&format!("not supports SetCate kind: {kind}"), "404-iam-cert-set-cate-kind-not-exist"))
    }
}
