...
impl IamCsCertApi {
    ...
    async fn add_or_modify_sync_third_integration_config(
        &self,
        req: Json<Vec<IamThirdIntegrationSyncAddReq>>,
        ctx: TardisContextExtractor,
        request: &Request,
    ) -> TardisApiResult<Void> {
        try_set_real_ip_from_req_to_ctx(request, &ctx.0).await?;
...
#[derive(Clone, Default)]
pub struct IamCsCertConfigOAuth2ServiceApi;

/// System Console Cert Config OAuth2 Service API
/// 系统控制台证书配置OAuth2服务API
#[poem_openapi::OpenApi(prefix_path = "/cs/oauth2-service", tag = "bios_basic::ApiTag::System")]
impl IamCsCertConfigOAuth2ServiceApi {
    /// Add OAuth2 Service Cert Conf
    /// 添加OAuth2服务证书配置
    #[oai(path = "/", method = "post")]
    async fn add_oauth2_service_cert(&self, add_req: Json<IamCertConfOAuth2ServiceAddOrModifyReq>, ctx: TardisContextExtractor, request: &Request) -> TardisApiResult<String> {
        try_set_real_ip_from_req_to_ctx(request, &ctx.0).await?;
        let mut funs = iam_constants::get_tardis_inst();
        funs.begin().await?;
        let resp = IamCertOAuth2ServiceServ::add_cert_conf(&add_req.0, &funs, &ctx.0).await?;
        funs.commit().await?;
        ctx.0.execute_task().await?;
        TardisResp::ok(resp)
    }
    ...
    async fn get_oauth2_service_cert(&self, id: Path<String>, ctx: TardisContextExtractor, request: &Request) -> TardisApiResult<IamCertConfOAuth2ServiceResp> {
        try_set_real_ip_from_req_to_ctx(request, &ctx.0).await?;
        let funs = iam_constants::get_tardis_inst();
        let resp = IamCertOAuth2ServiceServ::get_cert_conf(&id.0, &funs, &ctx.0).await?;
        ctx.0.execute_task().await?;
        TardisResp::ok(resp)
    }

    /// List OAuth2 Service Cert Confs
    /// 列出OAuth2服务证书配置
    #[oai(path = "/", method = "get")]
    ...
    /// Delete OAuth2 Service Cert Conf
    /// 删除OAuth2服务证书配置
    #[oai(path = "/:id", method = "delete")]
    async fn delete_oauth2_service_cert(&self, id: Path<String>, ctx: TardisContextExtractor, request: &Request) -> TardisApiResult<Void> {
        try_set_real_ip_from_req_to_ctx(request, &ctx.0).await?;
        let mut funs = iam_constants::get_tardis_inst();
        funs.begin().await?;
