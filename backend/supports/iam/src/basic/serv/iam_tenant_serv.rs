...
use crate::iam_enumeration::{
    IamCertExtKind, IamCertKernelKind, IamCertOAuth2Supplier, IamCertTokenKind, IamConfigDataTypeKind, IamConfigKind, IamRelKind, IamRoleKind, IamSetKind,
};
...
impl IamTenantServ {
    ...
    pub async fn add_tenant_agg(add_req: &IamTenantAggAddReq, funs: &TardisFunsInst, ctx: &TardisContext) -> TardisResult<(String, String, String)> {
        ...
        .await?;
        IamSearchClient::async_add_or_modify_account_search(&admin_id, Box::new(false), "", funs, &tenant_ctx).await?;
        IamSearchClient::async_add_or_modify_account_search(&audit_id, Box::new(false), "", funs, &tenant_ctx).await?;
    ...
    pub async fn find_id_rel_app(
        ...
        IamRelServ::find_to_id_rels(&IamRelKind::IamAppTenant, tenant_id, desc_sort_by_create, desc_sort_by_update, funs, ctx).await
    }
