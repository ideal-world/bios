...
/// 用户触达触发全局配置-租户控制台
pub struct ReachTriggerGlobalConfigCsApi;

/// Tenant Console Reach Trigger Global Config API
/// 租户控制台触达触发全局配置API
#[cfg_attr(feature = "simple-client", bios_sdk_invoke::simple_invoke_client(Client<'_>))]
#[poem_openapi::OpenApi(prefix_path = "/cs/trigger/global/config", tag = "bios_basic::ApiTag::Tenant")]
impl ReachTriggerGlobalConfigCsApi {
    ...
    /// Add or modify user reach trigger global config
    /// 保存用户触达触发实例配置
    #[oai(method = "put", path = "/")]
