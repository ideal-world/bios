...
pub(crate) async fn db_config_add(add_req: StatsSyncDbConfigAddReq, funs: &TardisFunsInst, ctx: &TardisContext) -> TardisResult<String> {
...
pub(crate) async fn db_config_list(funs: &TardisFunsInst, ctx: &TardisContext) -> TardisResult<Vec<StatsSyncDbConfigInfoResp>> {
...
pub(crate) async fn get_db_conn_by_cert_id(cert_id: &str, funs: &TardisFunsInst, ctx: &TardisContext) -> TardisResult<TardisRelDBlConnection> {
    let db_config = get_db_config(cert_id, funs, ctx).await?;
