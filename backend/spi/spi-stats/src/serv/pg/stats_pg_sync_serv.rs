...
use tardis::{
    ...
    db::{
        ...
        sea_orm::{FromQueryResult, QueryResult, Value},
    },
    log,
    regex::{self, Regex},
    TardisFunsInst,
...
use super::{stats_pg_conf_fact_serv, stats_pg_record_serv};
    ...
    if let Some(cert_id) = fact_conf.rel_cert_id {
        if let Some(sync_sql) = fact_conf.sync_sql {
            ...
            for db_source_record in db_source_list {
                ...
                stats_pg_record_serv::fact_record_load(fact_conf_key, &fact_record_key, add_req, funs, ctx, inst).await?;
            }
        } else {
