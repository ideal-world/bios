...
async fn test_ldap_account() -> TardisResult<()> {
    ...
    let (rs, _res) = ldap
        .search(&base_dn, Scope::Subtree, &format!("(sAMAccountName={})", test_username), requested_attrs.clone())
        .await
    ...
    let entries_multi: Vec<SearchEntry> = rs_multi.into_iter().map(SearchEntry::construct).collect();
    ...
    // 验证所有测试用户都被找到
    let mut found_usernames: Vec<String> = entries_multi.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_cn, _res_cn) = ldap
        .search(&base_dn, Scope::Subtree, &format!("(cn={})", test_username), vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_cn: Vec<SearchEntry> = rs_cn.into_iter().map(SearchEntry::construct).collect();
    ...
    let (rs_uid, _res_uid) = ldap
        .search(&base_dn, Scope::Subtree, &format!("(uid={})", test_username), vec!["sAMAccountName", "uid"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_uid: Vec<SearchEntry> = rs_uid.into_iter().map(SearchEntry::construct).collect();
    ...
    let (rs_mail, _res_mail) = ldap
        .search(&base_dn, Scope::Subtree, &format!("(mail={})", test_email), vec!["sAMAccountName", "mail"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_mail: Vec<SearchEntry> = rs_mail.into_iter().map(SearchEntry::construct).collect();
    ...
    let entries_emp: Vec<SearchEntry> = rs_emp.into_iter().map(SearchEntry::construct).collect();
    ...
    let entries_display: Vec<SearchEntry> = rs_display.into_iter().map(SearchEntry::construct).collect();
    ...
    let (rs_prefix, _res_prefix) = ldap
        .search(&base_dn, Scope::Subtree, "(sAMAccountName=test*)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_prefix: Vec<SearchEntry> = rs_prefix.into_iter().map(SearchEntry::construct).collect();

    assert!(entries_prefix.len() >= 3, "Prefix search should find at least 3 test users, found {}", entries_prefix.len());
    let mut found_prefix_usernames: Vec<String> = entries_prefix.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_suffix, _res_suffix) = ldap
        .search(&base_dn, Scope::Subtree, "(sAMAccountName=*user1)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_suffix: Vec<SearchEntry> = rs_suffix.into_iter().map(SearchEntry::construct).collect();
    ...
    let (rs_any, _res_any) = ldap
        .search(&base_dn, Scope::Subtree, "(sAMAccountName=*user*)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_any: Vec<SearchEntry> = rs_any.into_iter().map(SearchEntry::construct).collect();

    assert!(entries_any.len() >= 3, "Any position search should find at least 3 test users, found {}", entries_any.len());
    let mut found_any_usernames: Vec<String> = entries_any.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_prefix_any, _res_prefix_any) = ldap
        .search(&base_dn, Scope::Subtree, "(sAMAccountName=test*user*)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_prefix_any: Vec<SearchEntry> = rs_prefix_any.into_iter().map(SearchEntry::construct).collect();

    assert!(
        entries_prefix_any.len() >= 3,
        "Prefix + any search should find at least 3 test users, found {}",
        entries_prefix_any.len()
    );
    let mut found_prefix_any_usernames: Vec<String> = entries_prefix_any.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_any_suffix, _res_any_suffix) = ldap
        .search(&base_dn, Scope::Subtree, "(sAMAccountName=*user*1)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_any_suffix: Vec<SearchEntry> = rs_any_suffix.into_iter().map(SearchEntry::construct).collect();
    ...
    let (rs_prefix_suffix, _res_prefix_suffix) = ldap
        .search(&base_dn, Scope::Subtree, "(sAMAccountName=test*1)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_prefix_suffix: Vec<SearchEntry> = rs_prefix_suffix.into_iter().map(SearchEntry::construct).collect();
    ...
    let (rs_full_combo, _res_full_combo) = ldap
        .search(&base_dn, Scope::Subtree, "(sAMAccountName=test*user*1)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_full_combo: Vec<SearchEntry> = rs_full_combo.into_iter().map(SearchEntry::construct).collect();
    ...
    let (rs_multi_any, _res_multi_any) = ldap
        .search(&base_dn, Scope::Subtree, "(sAMAccountName=*user*test*)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_multi_any: Vec<SearchEntry> = rs_multi_any.into_iter().map(SearchEntry::construct).collect();

    assert!(
        entries_multi_any.len() >= 3,
        "Multiple any positions search should find at least 3 test users, found {}",
        entries_multi_any.len()
    );
    let mut found_multi_any_usernames: Vec<String> = entries_multi_any.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_mail_substr, _res_mail_substr) = ldap
        .search(&base_dn, Scope::Subtree, "(mail=*@example.com)", vec!["sAMAccountName", "mail"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_mail_substr: Vec<SearchEntry> = rs_mail_substr.into_iter().map(SearchEntry::construct).collect();

    assert!(
        entries_mail_substr.len() >= 3,
        "Mail substring search should find at least 3 test users, found {}",
        entries_mail_substr.len()
    );
    let mut found_mail_substr_usernames: Vec<String> = entries_mail_substr.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_display_substr, _res_display_substr) = ldap
        .search(&base_dn, Scope::Subtree, "(displayName=*用户*)", vec!["sAMAccountName", "displayName"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_display_substr: Vec<SearchEntry> = rs_display_substr.into_iter().map(SearchEntry::construct).collect();

    assert!(
        entries_display_substr.len() >= 3,
        "Displayname substring search should find at least 3 test users, found {}",
        entries_display_substr.len()
    );
    let mut found_display_substr_usernames: Vec<String> =
        entries_display_substr.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_and_substr, _res_and_substr) = ldap
        .search(&base_dn, Scope::Subtree, "(&(sAMAccountName=test*)(mail=*@example.com))", vec!["sAMAccountName", "mail"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_and_substr: Vec<SearchEntry> = rs_and_substr.into_iter().map(SearchEntry::construct).collect();

    assert!(
        entries_and_substr.len() >= 3,
        "AND combined substring search should find at least 3 test users, found {}",
        entries_and_substr.len()
    );
    let mut found_and_substr_usernames: Vec<String> = entries_and_substr.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_or_substr, _res_or_substr) = ldap
        .search(&base_dn, Scope::Subtree, "(|(sAMAccountName=*user1)(sAMAccountName=*user2))", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_or_substr: Vec<SearchEntry> = rs_or_substr.into_iter().map(SearchEntry::construct).collect();

    assert!(
        entries_or_substr.len() >= 2,
        "OR combined substring search should find at least 2 test users, found {}",
        entries_or_substr.len()
    );
    let mut found_or_substr_usernames: Vec<String> = entries_or_substr.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_not_eq, _res_not_eq) = ldap
        .search(&base_dn, Scope::Subtree, "(!(sAMAccountName=testuser1))", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_not_eq: Vec<SearchEntry> = rs_not_eq.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] NOT equality search found {} entries", entries_not_eq.len());
    let mut found_not_eq_usernames: Vec<String> = entries_not_eq.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let entries_not_substr: Vec<SearchEntry> = rs_not_substr.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] NOT substring search found {} entries", entries_not_substr.len());
    let mut found_not_substr_usernames: Vec<String> = entries_not_substr.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let entries_not_and: Vec<SearchEntry> = rs_not_and.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] NOT + AND search found {} entries", entries_not_and.len());
    let mut found_not_and_usernames: Vec<String> = entries_not_and.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let entries_not_or: Vec<SearchEntry> = rs_not_or.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] NOT + OR search found {} entries", entries_not_or.len());
    let mut found_not_or_usernames: Vec<String> = entries_not_or.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let entries_not_or2: Vec<SearchEntry> = rs_not_or2.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] NOT(OR) search found {} entries", entries_not_or2.len());
    let mut found_not_or2_usernames: Vec<String> = entries_not_or2.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let entries_not_mail: Vec<SearchEntry> = rs_not_mail.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] NOT mail search found {} entries", entries_not_mail.len());
    let mut found_not_mail_usernames: Vec<String> = entries_not_mail.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let entries_not_not: Vec<SearchEntry> = rs_not_not.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] NOT(NOT) search found {} entries", entries_not_not.len());
    let mut found_not_not_usernames: Vec<String> = entries_not_not.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let entries_complex: Vec<SearchEntry> = rs_complex.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] Complex NOT search found {} entries", entries_complex.len());
    let mut found_complex_usernames: Vec<String> = entries_complex.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_presence_sam, _res_presence_sam) = ldap
        .search(&base_dn, Scope::Subtree, "(sAMAccountName=*)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_presence_sam: Vec<SearchEntry> = rs_presence_sam.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] Presence sAMAccountName search found {} entries", entries_presence_sam.len());
    assert!(
        entries_presence_sam.len() >= 3,
        "Presence sAMAccountName search should find at least 3 test users, found {}",
        entries_presence_sam.len()
    );
    // 验证所有返回的条目都有 sAMAccountName 属性
    for entry in &entries_presence_sam {
        assert!(entry.attrs.contains_key("sAMAccountName"), "All entries should have sAMAccountName attribute");
    }
    let mut found_presence_sam_usernames: Vec<String> =
        entries_presence_sam.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_presence_mail, _res_presence_mail) = ldap
        .search(&base_dn, Scope::Subtree, "(mail=*)", vec!["sAMAccountName", "mail"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_presence_mail: Vec<SearchEntry> = rs_presence_mail.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] Presence mail search found {} entries", entries_presence_mail.len());
    assert!(
        entries_presence_mail.len() >= 3,
        "Presence mail search should find at least 3 test users, found {}",
        entries_presence_mail.len()
    );
    // 验证所有返回的条目都有 mail 属性
    for entry in &entries_presence_mail {
        assert!(entry.attrs.contains_key("mail"), "All entries should have mail attribute");
    }
    let mut found_presence_mail_usernames: Vec<String> =
        entries_presence_mail.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_presence_cn, _res_presence_cn) = ldap
        .search(&base_dn, Scope::Subtree, "(cn=*)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_presence_cn: Vec<SearchEntry> = rs_presence_cn.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] Presence cn search found {} entries", entries_presence_cn.len());
    assert!(
        entries_presence_cn.len() >= 3,
        "Presence cn search should find at least 3 test users, found {}",
        entries_presence_cn.len()
    );
    // 验证所有返回的条目都有 cn 属性
    for entry in &entries_presence_cn {
        assert!(entry.attrs.contains_key("cn"), "All entries should have cn attribute");
    }
    ...
    let (rs_presence_display, _res_presence_display) = ldap
        .search(&base_dn, Scope::Subtree, "(displayName=*)", vec!["sAMAccountName", "displayName"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_presence_display: Vec<SearchEntry> = rs_presence_display.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] Presence displayName search found {} entries", entries_presence_display.len());
    assert!(
        entries_presence_display.len() >= 3,
        "Presence displayName search should find at least 3 test users, found {}",
        entries_presence_display.len()
    );
    // 验证所有返回的条目都有 displayName 属性
    for entry in &entries_presence_display {
        assert!(entry.attrs.contains_key("displayName"), "All entries should have displayName attribute");
    }
    ...
    let (rs_presence_emp, _res_presence_emp) = ldap
        .search(&base_dn, Scope::Subtree, "(employeeNumber=*)", vec!["sAMAccountName", "employeeNumber"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_presence_emp: Vec<SearchEntry> = rs_presence_emp.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] Presence employeeNumber search found {} entries", entries_presence_emp.len());
    assert!(
        entries_presence_emp.len() >= 3,
        "Presence employeeNumber search should find at least 3 test users, found {}",
        entries_presence_emp.len()
    );
    // 验证所有返回的条目都有 employeeNumber 属性
    for entry in &entries_presence_emp {
        assert!(entry.attrs.contains_key("employeeNumber"), "All entries should have employeeNumber attribute");
    }
    ...
    let (rs_presence_and, _res_presence_and) = ldap
        .search(&base_dn, Scope::Subtree, "(&(sAMAccountName=*)(mail=*))", vec!["sAMAccountName", "mail", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_presence_and: Vec<SearchEntry> = rs_presence_and.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] Presence AND search found {} entries", entries_presence_and.len());
    assert!(
        entries_presence_and.len() >= 3,
        "Presence AND search should find at least 3 test users, found {}",
        entries_presence_and.len()
    );
    ...
    let mut found_presence_and_usernames: Vec<String> =
        entries_presence_and.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let entries_presence_or: Vec<SearchEntry> = rs_presence_or.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] Presence OR search found {} entries", entries_presence_or.len());
    assert!(
        entries_presence_or.len() >= 3,
        "Presence OR search should find at least 3 test users, found {}",
        entries_presence_or.len()
    );
    ...
    let mut found_presence_or_usernames: Vec<String> = entries_presence_or.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let entries_presence_complex: Vec<SearchEntry> = rs_presence_complex.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] Complex presence search found {} entries", entries_presence_complex.len());
    assert!(
        entries_presence_complex.len() >= 3,
        "Complex presence search should find at least 3 test users, found {}",
        entries_presence_complex.len()
    );
    ...
    let mut found_presence_complex_usernames: Vec<String> =
        entries_presence_complex.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let entries_presence_multi: Vec<SearchEntry> = rs_presence_multi.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] Multiple presence AND search found {} entries", entries_presence_multi.len());
    assert!(
        entries_presence_multi.len() >= 3,
        "Multiple presence AND search should find at least 3 test users, found {}",
        entries_presence_multi.len()
    );
    // 验证所有返回的条目同时有所有四个属性
    for entry in &entries_presence_multi {
        assert!(
            entry.attrs.contains_key("sAMAccountName") && entry.attrs.contains_key("cn") && entry.attrs.contains_key("mail") && entry.attrs.contains_key("displayName"),
            "All entries should have all four attributes: sAMAccountName, cn, mail, displayName"
        );
    }
    let mut found_presence_multi_usernames: Vec<String> =
        entries_presence_multi.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let entries2: Vec<SearchEntry> = rs2.into_iter().map(SearchEntry::construct).collect();
    ...
    assert!(root_dse_entry.attrs.contains_key("namingContexts"), "RootDSE should contain namingContexts attribute");
    ...
    assert_eq!(naming_contexts[0], base_dn, "namingContexts should match base DN: {}, got: {}", base_dn, naming_contexts[0]);
    ...
    // 4. vendorName - 供应商名称
    assert!(root_dse_entry.attrs.contains_key("vendorName"), "RootDSE should contain vendorName attribute");
    ...
    assert!(root_dse_entry.attrs.contains_key("vendorVersion"), "RootDSE should contain vendorVersion attribute");
    ...
    let entries_subschema: Vec<SearchEntry> = rs_subschema.into_iter().map(SearchEntry::construct).collect();
    ...
    let (rs_multi_attr, _res_multi_attr) = ldap
        .search("", Scope::Base, "(objectClass=*)", vec!["namingContexts", "subschemaSubentry", "supportedLDAPVersion"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_multi_attr: Vec<SearchEntry> = rs_multi_attr.into_iter().map(SearchEntry::construct).collect();
    ...
    // 验证返回了所有请求的属性
    assert!(multi_attr_entry.attrs.contains_key("namingContexts"), "RootDSE should contain namingContexts");
    assert!(multi_attr_entry.attrs.contains_key("subschemaSubentry"), "RootDSE should contain subschemaSubentry");
    assert!(multi_attr_entry.attrs.contains_key("supportedLDAPVersion"), "RootDSE should contain supportedLDAPVersion");
    assert_eq!(
    ...
    let entries_schema: Vec<SearchEntry> = rs_schema.into_iter().map(SearchEntry::construct).collect();
    ...
    assert!(schema_entry.attrs.contains_key("objectClass"), "Schema entry should contain objectClass attribute");
    ...
    assert!(schema_entry.attrs.contains_key("objectClasses"), "Schema entry should contain objectClasses attribute");
    ...
    assert!(object_classes_str.to_lowercase().contains("person"), "objectClasses should contain 'person' definition");
    assert!(
    ...
    assert!(schema_entry.attrs.contains_key("attributeTypes"), "Schema entry should contain attributeTypes attribute");
    ...
    assert!(schema_entry.attrs.contains_key("ldapSyntaxes"), "Schema entry should contain ldapSyntaxes attribute");
    ...
    assert!(schema_entry.attrs.contains_key("matchingRules"), "Schema entry should contain matchingRules attribute");
    ...
    assert!(schema_entry.attrs.contains_key("matchingRuleUse"), "Schema entry should contain matchingRuleUse attribute");
    ...
    assert!(schema_entry.attrs.contains_key("createTimestamp"), "Schema entry should contain createTimestamp attribute");
    ...
    assert!(schema_entry.attrs.contains_key("modifyTimestamp"), "Schema entry should contain modifyTimestamp attribute");
    ...
    let entries_schema_specific: Vec<SearchEntry> = rs_schema_specific.into_iter().map(SearchEntry::construct).collect();
    ...
    for attr in &requested_attrs {
        assert!(schema_entry_specific.attrs.contains_key(*attr), "Schema entry should contain requested attribute: {}", attr);
    }
    ...
    let (rs_schema_obj, _res_schema_obj) = ldap
        .search(&schema_dn, Scope::Base, "(objectClass=subschema)", vec!["objectClasses"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP schema search error: {e:?}"), "500-ldap-schema-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP schema search error: {e:?}"), "500-ldap-schema-search-error"))?;

    let entries_schema_obj: Vec<SearchEntry> = rs_schema_obj.into_iter().map(SearchEntry::construct).collect();
    ...
    let (rs_schema_attr, _res_schema_attr) = ldap
        .search(&schema_dn, Scope::Base, "(objectClass=subschema)", vec!["attributeTypes"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP schema search error: {e:?}"), "500-ldap-schema-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP schema search error: {e:?}"), "500-ldap-schema-search-error"))?;

    let entries_schema_attr: Vec<SearchEntry> = rs_schema_attr.into_iter().map(SearchEntry::construct).collect();
    ...
    let (rs_full, _res_full) = ldap
        .search(&base_dn, Scope::Subtree, "(objectClass=*)", vec!["sAMAccountName", "cn", "objectClass"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_full: Vec<SearchEntry> = rs_full.into_iter().map(SearchEntry::construct).collect();
    ...
    // 验证所有测试用户都在全量查询结果中
    let mut found_usernames_full: Vec<String> = entries_full.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_obj_eq1, _res_obj_eq1) = ldap
        .search(&base_dn, Scope::Subtree, "(objectClass=inetOrgPerson)", vec!["sAMAccountName", "cn", "objectClass"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_obj_eq1: Vec<SearchEntry> = rs_obj_eq1.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] objectClass=inetOrgPerson found {} entries", entries_obj_eq1.len());
    assert!(
        entries_obj_eq1.len() >= 3,
        "objectClass=inetOrgPerson should return at least 3 test users, found {}",
        entries_obj_eq1.len()
    );

    // 验证所有测试用户都在结果中
    let mut found_obj_eq1_usernames: Vec<String> = entries_obj_eq1.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_obj_eq2, _res_obj_eq2) = ldap
        .search(&base_dn, Scope::Subtree, "(objectClass=uidObject)", vec!["sAMAccountName", "cn", "objectClass"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_obj_eq2: Vec<SearchEntry> = rs_obj_eq2.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] objectClass=uidObject found {} entries", entries_obj_eq2.len());
    assert!(
        entries_obj_eq2.len() >= 3,
        "objectClass=uidObject should return at least 3 test users, found {}",
        entries_obj_eq2.len()
    );
    ...
    let (rs_obj_eq3, _res_obj_eq3) = ldap
        .search(&base_dn, Scope::Subtree, "(objectClass=top)", vec!["sAMAccountName", "cn", "objectClass"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_obj_eq3: Vec<SearchEntry> = rs_obj_eq3.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] objectClass=top found {} entries", entries_obj_eq3.len());
    assert!(
        entries_obj_eq3.len() >= 3,
        "objectClass=top should return at least 3 test users, found {}",
        entries_obj_eq3.len()
    );
    ...
    let (rs_obj_eq4, _res_obj_eq4) = ldap
        .search(&base_dn, Scope::Subtree, "(objectClass=person)", vec!["sAMAccountName", "cn", "objectClass"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_obj_eq4: Vec<SearchEntry> = rs_obj_eq4.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] objectClass=person found {} entries", entries_obj_eq4.len());
    assert!(
        entries_obj_eq4.is_empty(),
        "objectClass=person should return empty result (not in fixed list), found {}",
        entries_obj_eq4.len()
    );
    ...
    let (rs_obj_sub1, _res_obj_sub1) = ldap
        .search(&base_dn, Scope::Subtree, "(objectClass=inet*)", vec!["sAMAccountName", "cn", "objectClass"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_obj_sub1: Vec<SearchEntry> = rs_obj_sub1.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] objectClass=inet* found {} entries", entries_obj_sub1.len());
    assert!(
        entries_obj_sub1.len() >= 3,
        "objectClass=inet* should return at least 3 test users, found {}",
        entries_obj_sub1.len()
    );
    ...
    let entries_obj_sub2: Vec<SearchEntry> = rs_obj_sub2.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] objectClass=*Person found {} entries", entries_obj_sub2.len());
    assert!(
        entries_obj_sub2.len() >= 3,
        "objectClass=*Person should return at least 3 test users, found {}",
        entries_obj_sub2.len()
    );
    ...
    let (rs_obj_sub3, _res_obj_sub3) = ldap
        .search(&base_dn, Scope::Subtree, "(objectClass=*Object*)", vec!["sAMAccountName", "cn", "objectClass"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_obj_sub3: Vec<SearchEntry> = rs_obj_sub3.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] objectClass=*Object* found {} entries", entries_obj_sub3.len());
    assert!(
        entries_obj_sub3.len() >= 3,
        "objectClass=*Object* should return at least 3 test users, found {}",
        entries_obj_sub3.len()
    );
    ...
    let (rs_obj_sub4, _res_obj_sub4) = ldap
        .search(&base_dn, Scope::Subtree, "(objectClass=*Group*)", vec!["sAMAccountName", "cn", "objectClass"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_obj_sub4: Vec<SearchEntry> = rs_obj_sub4.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] objectClass=*Group* found {} entries", entries_obj_sub4.len());
    assert!(
        entries_obj_sub4.is_empty(),
        "objectClass=*Group* should return empty result (no matching fixed value), found {}",
        entries_obj_sub4.len()
    );
    ...
    let entries_obj_and: Vec<SearchEntry> = rs_obj_and.into_iter().map(SearchEntry::construct).collect();
    ...
    let entries_obj_or: Vec<SearchEntry> = rs_obj_or.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] objectClass OR filter found {} entries", entries_obj_or.len());
    assert!(
        entries_obj_or.len() >= 3,
        "objectClass OR filter should return at least 3 test users, found {}",
        entries_obj_or.len()
    );
    ...
    let entries_obj_not: Vec<SearchEntry> = rs_obj_not.into_iter().map(SearchEntry::construct).collect();
    ...
    assert!(
        entries_obj_not.len() >= 3,
        "NOT(objectClass=person) should return at least 3 test users, found {}",
        entries_obj_not.len()
    );
    ...
    let entries_base: Vec<SearchEntry> = rs_base.into_iter().map(SearchEntry::construct).collect();
    ...
    let (rs_onelevel, _res_onelevel) = ldap
        .search(&format!("ou=staff,{}", base_dn), Scope::OneLevel, "(objectClass=*)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_onelevel: Vec<SearchEntry> = rs_onelevel.into_iter().map(SearchEntry::construct).collect();
    ...
    assert!(
        entries_onelevel.len() >= 3,
        "OneLevel scope should return at least 3 test users, found {}",
        entries_onelevel.len()
    );

    let mut found_onelevel_usernames: Vec<String> = entries_onelevel.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    let (rs_subtree, _res_subtree) = ldap
        .search(&format!("ou=staff,{}", base_dn), Scope::Subtree, "(objectClass=*)", vec!["sAMAccountName", "cn"])
        .await
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?
        .success()
        .map_err(|e| tardis::basic::error::TardisError::internal_error(&format!("LDAP search error: {e:?}"), "500-ldap-search-error"))?;

    let entries_subtree: Vec<SearchEntry> = rs_subtree.into_iter().map(SearchEntry::construct).collect();

    info!("[Test] Subtree scope found {} entries", entries_subtree.len());
    // Subtree scope 应该返回 base DN 及其所有子级（递归）
    // 在当前实现中，应该返回所有用户
    assert!(
        entries_subtree.len() >= 3,
        "Subtree scope should return at least 3 test users, found {}",
        entries_subtree.len()
    );

    let mut found_subtree_usernames: Vec<String> = entries_subtree.iter().filter_map(|entry| entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned()).collect();
    ...
    assert!(
        entries_base.len() <= entries_onelevel.len(),
        "Base scope should return fewer or equal results than OneLevel"
    );
