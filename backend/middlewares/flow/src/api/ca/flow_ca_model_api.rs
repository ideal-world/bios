...
impl FlowCaModelApi {
    ...
    async fn copy_or_reference_model(
        ...
        for (_, rel_model_id) in req.0.rel_model_ids {
            ...
            FlowInstServ::batch_update_when_switch_model(None, &new_model.tag, &new_model.current_version_id, new_model.states.clone(), &new_model.init_state_id, &funs, &ctx.0).await?;
