pub mod kind {
    ...
    pub async fn peek_rbum_kind<'a>(id: &str, filter: &RbumBasicFilterReq, db: &TardisRelDBlConnection<'a>, cxt: &TardisContext) -> TardisResult<RbumKindSummaryResp> {
...
pub mod kind_attr {
    ...
    use crate::rbum::dto::rbum_kind_dto::{RbumKindDetailResp, RbumKindSummaryResp};
    ...
    pub async fn peek_rbum_kind_attr<'a>(id: &str, filter: &RbumBasicFilterReq, db: &TardisRelDBlConnection<'a>, cxt: &TardisContext) -> TardisResult<RbumKindAttrSummaryResp> {
        Ok(RbumKindAttrSummaryResp {
            id: "".to_string(),
            name: "".to_string(),
            label: "".to_string(),
            icon: "".to_string(),
    ...
    pub async fn get_rbum_kind_attr<'a>(id: &str, filter: &RbumBasicFilterReq, db: &TardisRelDBlConnection<'a>, cxt: &TardisContext) -> TardisResult<RbumKindAttrDetailResp> {
        Ok(RbumKindAttrDetailResp {
            ...
            rel_rbum_kind_name: "".to_string(),
            rel_app_id: "".to_string(),
            rel_app_name: "".to_string(),
            rel_tenant_id: "".to_string(),
            rel_tenant_name: "".to_string(),
            updater_id: "".to_string(),
            updater_name: "".to_string(),
            create_time: Utc::now(),
            update_time: Utc::now(),
            scope_kind: RbumScopeKind::Tag,
        })
    }
    ...
    pub async fn find_rbum_kind_attrs<'a>(
        ...
        db: &TardisRelDBlConnection<'a>,
        cxt: &TardisContext,
    ) -> TardisResult<TardisPage<RbumKindAttrDetailResp>> {
        Ok(TardisPage {
            page_size,
            page_number,
            total_size: 0,
