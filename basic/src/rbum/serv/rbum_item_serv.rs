...
pub mod item_attr {
    ...
    pub async fn add_rbum_item_attr<'a>(rbum_item_attr_add_req: &RbumItemAttrAddReq, db: &TardisRelDBlConnection<'a>, cxt: &TardisContext) -> TardisResult<String> {
        Ok(String::from(""))
    }
    ...
    pub async fn peek_rbum_item_attr<'a>(id: &str, filter: &RbumBasicFilterReq, db: &TardisRelDBlConnection<'a>, cxt: &TardisContext) -> TardisResult<RbumItemAttrSummaryResp> {
        Ok(RbumItemAttrSummaryResp {})
    }
    ...
    pub async fn find_rbum_item_attrs<'a>(
        ...
        db: &TardisRelDBlConnection<'a>,
        cxt: &TardisContext,
    ) -> TardisResult<TardisPage<RbumItemAttrDetailResp>> {
        Ok(TardisPage {
            page_size,
            page_number,
            total_size: 0,
