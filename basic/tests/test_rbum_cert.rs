...
use bios_basic::rbum::dto::filer_dto::{RbumBasicFilterReq, RbumCertConfFilterReq, RbumCertFilterReq};
...
async fn test_rbum_cert_conf(context: &TardisContext) -> TardisResult<()> {
    ...
    RbumCertConfServ::delete_rbum(&id, &funs, context).await?;
    assert!(RbumCertConfServ::get_rbum(&id, &RbumCertConfFilterReq::default(), &funs, context).await.is_err());
