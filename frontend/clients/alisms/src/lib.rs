...
impl SmsClient {
    ...
    fn signature(
        ...
        fn hmac256(key: &[u8], message: &str) -> TardisResult<Vec<u8>> {
            let mut mac = Hmac::<Sha256>::new_from_slice(key).map_err(|e| TardisError::internal_error(&format!("use data key on sha256 fail:{}", e), "500-reach-send-failed"))?;
            mac.update(message.as_bytes());
            let signature = mac.finalize();
            Ok(signature.into_bytes().to_vec())
        }
        let hashed_request_payload = if body_content.is_empty() { sha256_hex("") } else { sha256_hex(&body_content) };
        ...
        let datetime = DateTime::from_timestamp(now_time as i64, 0)
            .ok_or_else(|| TardisError::internal_error(&format!("Get datetime from timestamp failed: {}", now_time), "500-reach-send-failed"))?;
        let datetime_str = datetime.format("%Y-%m-%dT%H:%M:%SZ").to_string();

        // x-acs-signature-nonce
        let signature_nonce = generate_random_string(32);
        // 待签名请求头
        let sign_header_arr = &["host", "x-acs-action", "x-acs-content-sha256", "x-acs-date", "x-acs-signature-nonce", "x-acs-version"];
        ...
        // 5.计算签名
        let signature = hmac256(self.app_secret.as_bytes(), &string_to_sign)
            .map_err(|e| TardisError::internal_error(&format!("Calculate signature failed: {}", e), "500-reach-send-failed"))?;
        let data_sign = hex::encode(&signature);
        let auth_data = format!("ACS3-HMAC-SHA256 Credential={},SignedHeaders={},Signature={}", self.app_key, sign_headers, data_sign);
        // 6.构建 Authorization
    ...
    async fn send_request(
        ...
        // 构建请求
        let request = request_builder.build().map_err(|e| TardisError::internal_error(&format!("build request fail: {}", e), "500-reach-send-failed"))?;
        // 发起请求
        let response = self.inner.execute(request).await.map_err(|e| TardisError::internal_error(&format!("execute request fail: {}", e), "500-reach-send-failed"))?;
        // 返回结果
        Ok(response)
    }

    // 规范化请求
    pub async fn call_api(&self, method: Method, canonical_uri: &str, query_params: &[(&str, &str)], action: &str, version: &str, body: RequestBody) -> TardisResult<Response> {
        ...
        let response = self.send_request(method, &url, headers, query_params, &body, &body_content).await?;
...
pub fn percent_code(encode_str: &str) -> Cow<'_, str> {
    let encoded = utf8_percent_encode(encode_str, NON_ALPHANUMERIC).to_string().replace("+", "20%").replace("%5F", "_").replace("%2D", "-").replace("%2E", ".").replace("%7E", "~");
