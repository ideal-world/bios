...
// methods
impl Client {
    pub async fn send_message(&self, req: &SendCommonMessageRequest) -> TardisResult<CustomSmsResponse<SendCommonMessageResponse>> {
        let url = self.get_url("ums-rest-api/msgRest/sendCommonMsgJson");
        let resp = self.inner.post(url).json(&req).send().await?.json::<CustomSmsResponse<SendCommonMessageResponse>>().await?;
        Ok(resp)
    }
    pub async fn batch_send_messages(&self, req: &[&SendCommonMessageRequest]) -> TardisResult<CustomSmsResponse<Vec<SendCommonMessageResponse>>> {
        let url = self.get_url("ums-rest-api/msgRest/sendCommonMsgListJson");
        let resp = self.inner.post(url).json(&req).send().await?.json::<CustomSmsResponse<Vec<SendCommonMessageResponse>>>().await?;
        Ok(resp)
    }
}
