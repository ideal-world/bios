...
init_dev_env(){
    ...
    echo ""
    wget https://mirrors.bfsu.edu.cn/apache/apisix/2.13.1/apache-apisix-2.13.1-src.tgz
    tar -cvf apisix.tar apisix
    tar -xf apache-apisix-2.13.1-src.tgz -C apisix
    tar -xf apisix.tar
    rm apisix.tar
    rm apache-apisix-2.13.1-src.tgz
    echo "[6/9] make deps"
    echo ""
    cd apisix
    make deps
