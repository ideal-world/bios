...
impl IamCertServ {
    ...
    pub async fn get_kernel_cert(account_id: &str, rel_iam_cert_kind: &IamCertKernelKind, funs: &TardisFunsInst, ctx: &TardisContext) -> TardisResult<RbumCertSummaryWithSkResp> {
        let rel_rbum_cert_conf_id = if IamAccountServ::is_global_account(&ctx.owner, funs, ctx).await? {
            IamCertServ::get_cert_conf_id_by_kind(rel_iam_cert_kind.to_string().as_str(), None, funs).await?
        } else {
            Self::get_cert_conf_id_by_kind(rel_iam_cert_kind.to_string().as_str(), rbum_scope_helper::get_max_level_id_by_context(ctx), funs).await?
        };
