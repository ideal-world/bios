...
impl<'a> IamCpCertUserPwdServ {
    pub async fn login_by_user_pwd(login_req: &mut IamCpUserPwdLoginReq, funs: &TardisFunsInst<'a>) -> TardisResult<LoginResp> {
        let rbum_cert_conf_id = IamCertServ::get_id_by_code(&IamCertKind::UserPwd.to_string(), Some(&login_req.tenant_id), funs).await?;
        let (rbum_cert_id, _, rbum_item_id) = RbumCertServ::validate(&login_req.ak.0, &login_req.sk.0, &rbum_cert_conf_id, &login_req.tenant_id, funs).await?;
        let token = TardisFuns::crypto.key.generate_token()?;
        let token_kind = IamCertTokenKind::parse(&login_req.flag);
        let tardis_context = IamCertServ::package_tardis_context(
        ...
        .await?;
        IamCertTokenServ::add_cert(&token, token_kind, &rbum_item_id, &login_req.tenant_id, &rbum_cert_id, funs, &tardis_context).await?;
        IamCertServ::package_login_resp_from_context(funs, &tardis_context).await
    }
