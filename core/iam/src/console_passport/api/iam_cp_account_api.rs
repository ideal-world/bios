...
#[OpenApi(prefix_path = "/cp/account", tag = "bios_basic::Components::Iam")]
impl IamCpAccountApi {
    ...
    async fn modify(&self, mut modify_req: Json<IamCpAccountModifyReq>, cxt: TardisContextExtractor) -> TardisApiResult<Void> {
        ...
        tx.begin().await?;
        IamCpAccountServ::modify_account(&mut modify_req.0, &tx, &cxt.0).await?;
        tx.commit().await?;
        TardisResp::ok(Void {})
    }
    ...
    async fn get(&self, cxt: TardisContextExtractor) -> TardisApiResult<IamAccountDetailResp> {
        let result = IamCpAccountServ::get_account(&TardisFuns::reldb().conn(), &cxt.0).await?;
    ...
    async fn paginate_rel_roles(
        ...
        let result = IamCpAccountServ::paginate_rel_roles(page_number.0, page_size.0, desc_by_create.0, desc_by_update.0, &TardisFuns::reldb().conn(), &cxt.0).await?;
        TardisResp::ok(result)
    }
}
