...
impl IamCtCertConfApi {
    ...
    #[oai(path = "/", method = "get")]
    async fn find_cert_conf(&self, ctx: TardisContextExtractor) -> TardisApiResult<Vec<RbumCertConfDetailResp>> {
        let funs = iam_constants::get_tardis_inst();
        let result =
            IamCertServ::find_cert_conf_detail_without_token_kind(None, None, None, true, Some(IamTenantServ::get_id_by_ctx(&ctx.0, &funs)?), None, None, &funs, &ctx.0).await?;
    ...
    async fn add_cert_conf_mail_vcode(&self, add_req: Json<IamMailVCodeCertConfAddOrModifyReq>, ctx: TardisContextExtractor) -> TardisApiResult<Void> {
        let mut funs = iam_constants::get_tardis_inst();
        funs.begin().await?;
        IamCertMailVCodeServ::add_cert_conf(&add_req.0, Some(IamTenantServ::get_id_by_ctx(&ctx.0, &funs)?), &funs, &ctx.0).await?;
        funs.commit().await?;
        TardisResp::ok(Void {})
    }
    ...
    async fn add_cert_conf_phone_vcode(&self, add_req: Json<IamPhoneVCodeCertConfAddOrModifyReq>, ctx: TardisContextExtractor) -> TardisApiResult<Void> {
        let mut funs = iam_constants::get_tardis_inst();
        funs.begin().await?;
        IamCertPhoneVCodeServ::add_cert_conf(&add_req.0, Some(IamTenantServ::get_id_by_ctx(&ctx.0, &funs)?), &funs, &ctx.0).await?;
        funs.commit().await?;
        TardisResp::ok(Void {})
    }
