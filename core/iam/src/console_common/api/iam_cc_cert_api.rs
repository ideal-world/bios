...
impl IamCcCertApi {
    ...
    async fn rest_password(&self, account_id: Path<String>, mut modify_req: Json<IamUserPwdCertRestReq>, cxt: TardisContextExtractor) -> TardisApiResult<Void> {
        ...
        IamCertUserPwdServ::reset_sk(&mut modify_req.0, &account_id.0, &rbum_cert_conf_id, &funs, &cxt.0).await?;
        funs.commit().await?;
        TardisResp::ok(Void {})
    }
}
