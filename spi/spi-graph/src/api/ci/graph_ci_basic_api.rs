...
impl GraphCiRelApi {
    ...
    async fn add_rel(&self, add_or_modify_req: Json<GraphRelAddReq>, ctx: TardisContextExtractor) -> TardisApiResult<Void> {
    ...
    async fn find_versions(&self, tag: Query<String>, key: Query<String>, ctx: TardisContextExtractor) -> TardisApiResult<Vec<GraphNodeVersionResp>> {
        let funs = crate::get_tardis_inst();
    ...
    async fn find_rels(&self, from_key: Query<String>, from_version: Query<String>, depth: Query<Option<u8>>, ctx: TardisContextExtractor) -> TardisApiResult<GraphRelDetailResp> {
        let funs = crate::get_tardis_inst();
