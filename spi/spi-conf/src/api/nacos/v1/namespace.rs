...
impl ConfNacosV1NamespaceApi {
    ...
    async fn get_namespace_list(&self, #[oai(name = "accessToken")] access_token: Query<String>, request: &Request) -> poem::Result<Json<NacosResponse<Vec<NamespaceItem>>>> {
    ...
    async fn create_namespace(
        &self,
        form: Form<NacosCreateNamespaceRequest>,
    ...
    #[oai(path = "/", method = "put")]
    async fn edit_namespace(&self, form: Form<NacosEditNamespaceRequest>, #[oai(name = "accessToken")] access_token: Query<String>, request: &Request) -> poem::Result<Json<bool>> {
        let funs = request.tardis_fun_inst();
        let ctx = jwt_validate(&access_token.0, &funs).await?;
        let mut attribute = form.0.into();
    ...
    async fn delete_namespace(
        &self,
        form: Form<NacosDeleteNamespaceRequest>,
        #[oai(name = "accessToken")] access_token: Query<String>,
        request: &Request,
    ) -> poem::Result<Json<bool>> {
        let funs = request.tardis_fun_inst();
