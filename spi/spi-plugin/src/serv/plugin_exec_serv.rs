        ...
        if let Some(spi_api) = &spi_api {
            ...
            match spi_api.http_method {
                crate::plugin_enumeration::PluginApiMethodKind::GET => {
                    result = funs.web_client().get_to_str(url, headers.clone()).await?;
                }
                crate::plugin_enumeration::PluginApiMethodKind::PUT => {
                    result = funs.web_client().put_str_to_str(url, &TardisFuns::json.obj_to_string(&exec_req.body)?, headers.clone()).await?;
                }
                crate::plugin_enumeration::PluginApiMethodKind::POST => {
                    result = funs.web_client().post_str_to_str(url, &TardisFuns::json.obj_to_string(&exec_req.body)?, headers.clone()).await?;
                }

                crate::plugin_enumeration::PluginApiMethodKind::DELETE => {
                    result = funs.web_client().delete(url, headers.clone()).await?;
                }
                crate::plugin_enumeration::PluginApiMethodKind::PATCH => {
                    result = funs.web_client().patch_str_to_str(url, &TardisFuns::json.obj_to_string(&exec_req.body)?, headers).await?;
                }
            }
